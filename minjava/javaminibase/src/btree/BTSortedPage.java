...
public class BTSortedPage extends HFPage {
    ...
    protected RID insertRecord(KeyDataEntry entry)
        ...
        try {
            ...
            short tupleFldCnt = 0;
            AttrType[] tupleAttrType = new AttrType[0];
            short[] tupleStrSizes = new short[0];

            if (entry.data instanceof LeafData)
                nType = NodeType.LEAF;
            else if (entry.data instanceof ClusteredLeafData) {
                nType = NodeType.LEAF_CLUSTERED;
                Tuple tuple = ((ClusteredLeafData)entry.data).getData();
                tupleFldCnt = tuple.noOfFlds();
                tupleAttrType = tuple.getTypes();
                tupleStrSizes = tuple.getStrSizes();
            }
