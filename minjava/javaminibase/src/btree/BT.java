...
public class BT implements GlobalConst {
     ...
     *                               nor NodeType.INDEX.
     */
    protected final static int getDataLength(short pageType, short tupleFldCnt, AttrType[] tupleAttrType, short[] tupleStrSizes)
            throws NodeNotMatchException {
        if (pageType == NodeType.LEAF)
            return 8;
        else if (pageType == NodeType.LEAF_CLUSTERED) {
    ...
    protected final static int getKeyDataLength(KeyClass key, short pageType)
            ...
            IOException {
        return getKeyDataLength(key, pageType, (short) 0, new AttrType[0], new short[0]);
    }

    protected final static int getKeyDataLength(KeyClass key, short pageType, short tupleFldCnt, AttrType[] tupleAttrType, short[] tupleStrSizes)
            throws KeyNotMatchException,
            NodeNotMatchException,
            IOException {
        return getKeyLength(key) + getDataLength(pageType, tupleFldCnt, tupleAttrType, tupleStrSizes);
    }
